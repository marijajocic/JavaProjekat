...
public class Imenik {
	...
	public boolean dodavanjeKontakta(Kontakt kontakt) {
		...
		Kontakt noviKontakt = new Kontakt(kontakt.getId(), kontakt.getIme(), kontakt.getPrezime(),
				kontakt.getNazivRadnogMesta(), kontakt.getBrojProstorije(), kontakt.getBrojLokala(), kontakt.getDatumUnosa());
	...
	public Kontakt brisanjeKontakta(int id) {
		...
		if(indeks != -1) {
			...
			/*
			 * return this.listaKontakta.remove(indeks);
			 */
		}
		return null;
	}
	...
	public Kontakt izmenaKontakta(Kontakt kontakt) {
		...
		for (int i = 0; i < this.listaKontakta.size(); i++) {
			...
			Kontakt kontaktIzListe = this.listaKontakta.get(i); //očitavamo objekat tipa Kontakt
			int idKontaktaIzListe = kontaktIzListe.getId(); //iz njega dobavljamo njegov id pomoću metode getId() koja je definisana u klasi Kontakt
			if(idKontaktaIzListe == kontakt.getId()) {
	...
	public void save(String path) {
		 ...
		 * 
		 * identifikacioniBroj;imeKontakta;prezimeKontakta;nazivRadnogMesta;brojProstorije;brojLokala;datum
		 */
		...
		for (int i = 0; i < this.listaKontakta.size(); i++) {
			...
			LocalDate datumUnosa = kontakt.getDatumUnosa();
			...
			/*String line = this.listaKontakta.get(i).getId() + ";" + this.listaKontakta.get(i).getIme() + ";"
							...
							+ dtf.format(this.listaKontakta.get(i).getDatumUnosa());
			lines.add(line);*/
		}
	...
	public void load(String path) {
		...
		try {
			...
			System.out.println("U�?itan datum nije u odgovarajućem formatu.");
		}
	...
	public void ispisKontakataSaZadatimUslovima(String ime, String prezime, String nazivRadnogMesta) {
		...
		for (int i = 0; i < this.listaKontakta.size(); i++) {
			 ...
			 * Koristimo metodu startsWith koju poseduje klasa String da proverimo da li naziv radnog mesta kontakta po�?inje sa nazivom koji se prosleđuje metodi.
			 * Isti efekat se postiže i zakomentarisanim kodom:
			 */
			/*			Kontakt kontaktIzListe = this.listaKontakta.get(i); //očitavamo objekat tipa Kontakt
	...
	public void ispisKontakataUIntervalu(LocalDate datumPocetka, LocalDate datumKraja) {
		...
		for (int i = 0; i < this.listaKontakta.size(); i++) {
			/*
			 * Svi kontakti koji su kreirani u zadatom intervalu su kreirani posle početka intervala (datumPocetka)
			 * i pre kraja intervala (datumKraja).
			 * Isti efekat se postiže i zakomentarisanim kodom:
			 */
			/*Kontakt kontaktIzListe = this.listaKontakta.get(i); //očitavamo objekat tipa Kontakt
			LocalDate datumKontaktaIzListe = kontaktIzListe.getDatumUnosa(); //iz njega dobavljamo datum unosa kontakta pomoću metode getDatumUnosa() koja je definisana u klasi Kontakt
			if (datumKontaktaIzListe.compareTo(datumPocetka) > 0 &&
					datumKontaktaIzListe.compareTo(datumKraja) < 0) {
				Kontakt kontaktKojiZadovoljavaUslove = this.listaKontakta.get(i);
				System.out.println(kontaktKojiZadovoljavaUslove);
			}*/
			if ((this.listaKontakta.get(i)).getDatumUnosa().compareTo(datumPocetka) > 0 &&
					(this.listaKontakta.get(i)).getDatumUnosa().compareTo(datumKraja) < 0) {
				System.out.println(this.listaKontakta.get(i));
			}
		}
	}
